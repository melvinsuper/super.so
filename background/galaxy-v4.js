(function () {
  /*
    Shared GitHub/jsDelivr version.

    Each Super user adds their own site URL in the script tag using:

    data-super-galaxy-sites="https://example.super.site"

    Multiple sites are also supported:

    data-super-galaxy-sites="https://example.super.site,https://www.example.com"
  */

  function normalizeGalaxyHostname(value) {
    if (!value) return "";
  ...
  function getGalaxyScriptElement() {
  ...
  function getAllowedGalaxyHostnames() {
    var script = getGalaxyScriptElement();
    var rawSites = "";

    if (script) {
      rawSites =
        script.getAttribute("data-super-galaxy-sites") ||
        script.getAttribute("data-super-galaxy-hosts") ||
        "";
    }

    if (
      !rawSites &&
      window.SuperReactBitsGalaxy &&
      Array.isArray(window.SuperReactBitsGalaxy.allowedSites)
    ) {
      return window.SuperReactBitsGalaxy.allowedSites
        .map(normalizeGalaxyHostname)
        .filter(Boolean);
    }
  ...
  function isSuperDashboardOrPreview() {
    var currentHostname = normalizeGalaxyHostname(window.location.hostname);

    if (currentHostname === "app.super.so") return true;
  ...
  var allowedHostnames = getAllowedGalaxyHostnames();
  var currentHostname = normalizeGalaxyHostname(window.location.hostname);

  if (isSuperDashboardOrPreview()) return;
  if (!allowedHostnames.length) return;
  if (allowedHostnames.indexOf(currentHostname) === -1) return;
  ...
  function startGalaxy() {
    ...
    function keepGalaxyMounted() {
      if (!document.body) return;

      var existingGalaxy = document.getElementById("super-reactbits-galaxy-bg");
      ...
      if (existingGalaxy !== wrapper) {
        existingGalaxy.remove();
        document.body.prepend(wrapper);
      }

      if (!document.getElementById("super-reactbits-galaxy-style")) {
        document.head.appendChild(style);
      }
    }

    if (window.MutationObserver) {
      var galaxyMountObserver = new MutationObserver(function () {
        keepGalaxyMounted();
      });

      galaxyMountObserver.observe(document.documentElement, {
        childList: true,
        subtree: true
      });
    }

    setInterval(keepGalaxyMounted, 1000);
