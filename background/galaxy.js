(function () {
  /*
    Super ReactBits Galaxy Background
    CDN-safe version.

    How this works:
    - Keep this full file hosted once through jsDelivr.
    - Each Super user adds a small config snippet before the CDN script.
    - The effect only runs on the URL/hostname added in allowedUrls.
    - The effect does not run inside the Super dashboard preview/editor.
  */

  var config = window.SuperGalaxyConfig || {};
  ...
  function getAllowedHostnames() {
    var allowedUrls = config.allowedUrls || config.allowedHostnames || [];
  ...
  function isSuperDashboardContext() {
    var currentHostname = window.location.hostname.toLowerCase();
    var referrer = document.referrer || "";

    if (currentHostname === "app.super.so") return true;
    if (currentHostname.endsWith(".app.super.so")) return true;
    if (referrer.indexOf("app.super.so") !== -1) return true;

    /*
      Super dashboard preview usually renders the site inside an iframe.
      This prevents the background from affecting the dashboard/editor.
    */
  ...
  function shouldRunOnThisSite() {
    ...
    /*
      Safety rule:
      If the user did not add their site URL in the config snippet,
      the effect will not run anywhere.
    */
    if (!allowedHostnames.length) return false;

    var currentHostname = toHostname(window.location.hostname);

    return allowedHostnames.indexOf(currentHostname) !== -1;
  }

  if (isSuperDashboardContext()) return;
